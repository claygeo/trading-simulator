// backend/src/services/simulation/types.ts - COMPLETE: Communication Layer Types + State Management
...
// COMMUNICATION LAYER: Simulation state types for proper synchronization
export type SimulationStatus = 'initializing' | 'ready' | 'running' | 'paused' | 'stopped' | 'error' | 'reset';

export interface SimulationStateSync {
  id: string;
  status: SimulationStatus;
  isRunning: boolean;
  isPaused: boolean;
  lastStatusUpdate: number;
  pendingStateChanges: string[];
  stateTransitionInProgress: boolean;
}

// COMMUNICATION LAYER: WebSocket message types for state coordination
export interface WebSocketStateMessage {
  type: 'state_update' | 'state_sync' | 'state_change_request' | 'state_change_response';
  simulationId: string;
  currentState: SimulationStateSync;
  requestedState?: SimulationStatus;
  timestamp: number;
  clientId?: string;
}

// COMMUNICATION LAYER: API response standardization
export interface StandardApiResponse<T = any> {
  ...
  timestamp: number;
  simulationId?: string;
  stateSync?: SimulationStateSync;
  coordinatorStatus?: 'active' | 'inactive' | 'error';
}

// COMMUNICATION LAYER: Chart data validation types
...
export interface ValidatedCandleData {
  candles: PricePoint[];
  validation: ChartDataValidation;
  processed: boolean;
  coordinatorGenerated: boolean;
}
...
export interface CandleData extends PricePoint {
  ...
  // COMMUNICATION LAYER: Additional sync fields
  coordinatorGenerated?: boolean; // generated by coordinator
  validated?: boolean; // passed validation
  lastUpdated?: number; // when last updated
}

// COMMUNICATION LAYER: CandleUpdateCallback interface for coordination
export interface CandleUpdateCallback {
  ...
  clearCandles(simulationId: string): Promise<void>;
  ensureCleanStart(simulationId: string): Promise<void>;
  // NEW: State synchronization methods
  getValidatedCandles(simulationId: string): ValidatedCandleData;
  forceSyncState(simulationId: string): Promise<boolean>;
  getCoordinatorStatus(simulationId: string): 'active' | 'inactive' | 'error';
}
...
export interface CandleManagerStats {
  ...
  // COMMUNICATION LAYER: Sync status
...
export interface SimulationState {
  ...
  // COMMUNICATION LAYER: State sync fields
  stateSync?: SimulationStateSync;
}
...
// COMMUNICATION LAYER: State transition management
...
export interface StateManager {
  getCurrentState(simulationId: string): SimulationStateSync | null;
  requestStateChange(simulationId: string, newState: SimulationStatus, clientId?: string): Promise<boolean>;
  validateStateTransition(from: SimulationStatus, to: SimulationStatus): boolean;
  getStateHistory(simulationId: string): StateTransition[];
  clearState(simulationId: string): void;
}

// COMMUNICATION LAYER: Pause/Resume specific types
...
// COMMUNICATION LAYER: Reset specific types
export interface ResetRequest {
  simulationId: string;
  options: {
    clearAllData: boolean;
    resetState: 'complete' | 'partial';
    generateNewPrice: boolean;
    preserveTraders: boolean;
  };
  timestamp: number;
  clientId?: string;
}
...
export interface ResetResponse {
  success: boolean;
  simulationId: string;
  newState: ExtendedSimulationState;
  clearedData: {
    candlesCleared: number;
...
export const SIMULATION_CONSTANTS = {
  ...
  // COMMUNICATION LAYER: State management constants
  STATE_MANAGEMENT: {
    STATE_SYNC_INTERVAL: 1000, // 1 second state sync
    MAX_STATE_TRANSITIONS_PER_MINUTE: 60, // prevent rapid state changes
    STATE_TRANSITION_TIMEOUT: 5000, // 5 second timeout for state changes
    DUPLICATE_MESSAGE_WINDOW: 2000, // 2 second window for duplicate detection
    MAX_PENDING_STATE_CHANGES: 3, // maximum queued state changes
    WEBSOCKET_RECONNECT_DELAY: 1000, // 1 second reconnect delay
    API_REQUEST_TIMEOUT: 10000 // 10 second API timeout
  }
};
...
// COMMUNICATION LAYER: CandleManager interface for coordination
export interface ICandleManager {
  ...
  // NEW: Communication layer methods
  validateCandles(): ChartDataValidation;
  getValidatedCandles(): ValidatedCandleData;
  syncWithCoordinator(): Promise<boolean>;
}
...
// COMMUNICATION LAYER: WebSocket message types for candle coordination and state sync
export interface WebSocketMessage {
  type: 'price_update' | 'trade' | 'candle_update' | 'simulation_status' | 'tps_metrics' | 'connection_test' | 'state_sync' | 'pause_response' | 'reset_response';
  timestamp: number;
  data?: any;
  simulationId?: string;
  messageId?: string; // for duplicate detection
...
// COMMUNICATION LAYER: Pause/Resume response message
export interface PauseResponseMessage extends WebSocketMessage {
  type: 'pause_response';
  data: PauseResumeResponse;
}

// COMMUNICATION LAYER: Reset response message
export interface ResetResponseMessage extends WebSocketMessage {
  type: 'reset_response';
  data: ResetResponse;
}

// COMMUNICATION LAYER: State synchronization message
export interface StateSyncMessage extends WebSocketMessage {
  ...
  data: {
    ...
    currentState: SimulationStateSync;
    coordinatorStatus: 'active' | 'inactive' | 'error';
    candleValidation?: ChartDataValidation;
    forcedSync?: boolean;
  };
}
...
export interface PerformanceMetrics {
  ...
  timestamp: number;
  // COMMUNICATION LAYER: Additional metrics
  websocketConnections: number;
  messageProcessingRate: number;
  stateTransitionsPerMinute: number;
...
// COMMUNICATION LAYER: Client connection tracking
...
// COMMUNICATION LAYER: Message queue management
...
// COMMUNICATION LAYER: Broadcast management
export interface BroadcastManager {
  registerClient(ws: any, simulationId: string): void;
  removeClient(ws: any): void;
  sendDirectMessage(simulationId: string, message: any): void;
  broadcastToAll(message: any): void;
  broadcastToSimulation(simulationId: string, message: any): void;
  getClientCount(simulationId?: string): number;
  cleanup(): void;
  // NEW: State synchronization
  syncState(simulationId: string): Promise<void>;
  forceBroadcastState(simulationId: string, stateSync: SimulationStateSync): void;
}

// Export all enhanced types for communication layer coordination
export * from './types';